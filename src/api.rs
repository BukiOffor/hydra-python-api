...
impl IopSdk {
    ...
    pub fn get_wallet_address<'a>(&self,data: String, account: i32, idx:i32, network: &'a str) -> Result<String, IopError> {
    ...
    pub fn get_hyd_vault( &self, phrase: String, password: String, network: String, account: i32) -> Result<String,IopError> {
        let mut vault =
            match Vault::create(None, phrase, "", &password){
                Ok(vault) => vault,
    ...
    pub fn get_morpheus_vault( &self, phrase: String, password: String) -> Result<String, IopError> {
        ...
        match vault {
            ...
            Err(_err) => {
                Err(IopError::CouldNotCreateMorpheusVault)
                }
            }
        }
    ...
    fn deserialize_hydra<'a>(
        ...
        let wallet_private = match wallet.private(&unlock_password){
            Ok(mut wallet_private) => {
                match wallet_private.key_mut(idx) {
    ...
    pub fn generate_did_by_morpheus(&self, data: String, password: String, idx:i32) -> Result<String,IopError> {
    ...
    pub fn sign_witness_statement(&self, vault: String, password: String, data: &str, idx:i32) -> Result<String, IopError> {
        let (pk, kpub) = self.deserialize_morpheus(vault, password, idx)?;
        let private_key = pk.key_by_pk(&kpub).unwrap().private_key();
        let signer = PrivateKeySigner::new(private_key);
